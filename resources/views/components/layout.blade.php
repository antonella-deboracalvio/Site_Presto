...
<body>
    ...
    <div class="offcanvas offcanvas-end" data-bs-backdrop="static" tabindex="-1" id="offcanvasRight"
        ...
        <div class="offcanvas-header">
            ...
            <button id="btn-close" type="button" class="my-bg-quar btn-info-custom btn-close"
                        ...
                        <div id="collapseOne" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                            <div class="accordion-body">
                                <form id="loginForm" method="POST" action="{{ route('login') }}">
                                    ...
                                    <button type="submit"
                                        class="btn btn-info-custom my-bg-quar">{{ __('ui.login') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                        ...
                        <div id="collapseThree" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
                            <div class="accordion-body">
                                <form method="POST" action="{{ route('register') }}">
                                    ...
                                    <button type="submit"
                                        class="btn btn-info-custom my-bg-quar">{{ __('ui.register') }}</button>
                                </form>
                            </div>
                        </div>
                    </div>
                ...
                <div class="container">
                    <div class="row">
                        <div class="col-12 border rounded p-0">
                            <div class="w-100">
                                <a class="nav-link text-white text-center border-bottom py-3 my-bg-main btn-offcanvas-logged rounded-top"
                                    href="{{ route('createArticle') }}">{{ __('ui.insert an article') }}</a>

                                @if (Auth::user()->is_revisor)
                                    @if (\App\Models\Article::toBeRevisedCount() > 0)
                                        <span
                                            class="notifica-revisore-offcanvas translate-middle badge rounded-pill bg-danger">
                                            {{ \App\Models\Article::toBeRevisedCount() }}
                                        </span>
                                    @endif
                                    <a href="{{ route('indexRevisor') }}"
                                        class="nav-link text-white my-bg-main position position-relative w-sm-25 text-center py-3 border-bottom btn-offcanvas-logged">{{ __('ui.revisor zone') }}</a>
                                @endif
                            ...
                            <form class="" action="{{ route('logout') }}" method="post">
                                @csrf
                                <button class="rounded-bottom py-2 bg-danger text-white border-0 w-100"
                                    type="submit">{{ __('ui.logout') }}<i
                                        class="fa-solid fa-person-through-window ms-1"></i></button>
                            </form>
